function [strain_11, strain_22, strain_12] = fDIC_strain_method_1(nodeCoords_X,nodeCoords_Y,displacements_X,displacements_Y, num_row, num_col, Image_size)
% 9 nodes finite element analysis for strain--- not very good 
  [local_elem,local_elem_x,local_elem_y] = fDIC_shape_positions_9nodes(nodeCoords_X,nodeCoords_Y);
  [local_disp]                           = fDIC_shape_displacements_9nodes(displacements_X,displacements_Y);
    ...
    for I=1:(num_col-2)*(num_row-2) % the two rows and two colums from the edges of top right not be tested as they have been included in the 3x3 matrix
        parfor K=1:numel(xi_X)
            % SHAPE FUNCTIONS AND DERIVATES WRT LOCAL COORDS
            [N]      = fDIC_shape_function_9nodes(xi_X(K),xi_Y(K));
            [dN_dXi] = fDIC_shape_function_deriviative_9nodes(xi_X(K),xi_Y(K));
