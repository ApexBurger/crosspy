...
def run_DIC(d,imnos=[0,1],hs=False, cc_t=0., cores=1,ffttype='fftw_numpy',cormeth="efficient"):
    ...
    if hs == True:
        ...
        results = Parallel(n_jobs=cores, verbose=5)(delayed(subset_compare) \
            (subset_n=i, d=d, imnos=imnos, prepared_ffts=prepared_ffts, hs=True, cc_t=cc_t,cormeth=cormeth) for i in range(d.n_subsets))
